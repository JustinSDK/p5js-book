...
function draw() {
  ...
  strokeWeight(5);
  translate(width / 2 - maze.columns / 2 * cellWidth,
            height / 2 - maze.rows / 2 * cellWidth);
  
  // n 尚未等於細胞總數，畫出探索路徑
  if(n !== maze.cells.length + 1) {
    // 畫出探索路徑
    drawTube(maze, cellWidth, n);
    n++;
  }
  else {
    // 畫出迷宮
...
// 指定Maze實例與細胞大小
function drawTube(maze, cellWidth, n) {
  // 繪出探索路徑
  const tube = findTube(maze, n);
  for(let cell of tube) {
    const px = cell.x * cellWidth;
    const py = cell.y * cellWidth;
    push();
    noStroke();
    // 基於位置來繪製顏色，從 50 開始是希望顏色不要太暗
    fill(px % 255 + 50, py % 255 + 50, 0);
    translate(px, py);
